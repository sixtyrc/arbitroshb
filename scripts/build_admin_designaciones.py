...
def update_api_js():
    ...
    new_methods = ""
    if "getArbitros:" not in content:
        new_methods += "  getArbitros: () => api.get('/arbitros/'),\n"
    if "createDesignacion:" not in content:
        new_methods += "  createDesignacion: (data) => api.post('/designaciones/', data),\n"
    if "deleteDesignacion:" not in content:
        new_methods += "  deleteDesignacion: (id) => api.delete(`/designaciones/${id}/`),\n"
    ...
    if new_methods:
        ...
        with open(path, "w", encoding="utf-8") as f:
            f.write(content)
        print("Updated api.js for Designaciones")

def update_admin_designaciones_jsx():
    path = r"d:\Proyectos\Arbitros\frontend\src\views\admin\AdminDesignaciones.jsx"
...
"""
    with open(path, "w", encoding="utf-8") as f:
        f.write(jsx)
    print("Updated AdminDesignaciones.jsx")

if __name__ == "__main__":
    update_api_js()
    update_admin_designaciones_jsx()
    print("Build step for Admin Designaciones completed!")
